...
malaysia_sf <- malaysia_sf %>%
  filter(name != 'Sabah') %>%
  filter(name != 'Sarawak') %>%
  filter(name != 'W.P. Labuan') %>%
  mutate(state = replace(name, name == 'W.P. Kuala Lumpur', 'Kuala Lumpur'))
```
...
```{r}
# Set tmap mode to view (interactive) or plot (static)
tmap_mode("plot")

# Create a choropleth map of crime rates across Malaysian states
tm_shape(crime_district_malaysia) +
  tm_polygons("crime_rate", 
              ...
              style = "quantile") +
  tm_borders() +
  tm_layout(title = "Choropleth Map of Crime Rates in Malaysian States",
            legend.outside = TRUE)
...
```{r}
# Histogram of crime rates across states
ggplot(crime_district, aes(x = crime_district_malaysia)) +  # Adjust column name to reflect your dataset
  geom_histogram(binwidth = 5, fill = "blue", color = "black") +
  labs(title = "Distribution of Crime Rates in Malaysian States",
       x = "Crime Rate",
       y = "Frequency") +
  theme_minimal()
